...
%define constants
q = 1.6e-19; hbar = 1.06e-34;h = 2*pi*hbar;m = 9.1e-31;zplus=i*1e-3; 
%t has been given as a parameter
t = -3;

f1 =1; f2 = 0;
fprintf(1,'Enter the type of device you want to simulate GNR/CNT\n');
fprintf(1,'1 for GNR\n');
fprintf(1,'2 for CNT\n');
device = input('Valid inputs are 1 or 2 : ');
...
if (~((type == 1)||(type == 2)))
    MATLAB code to generate Transmission and Sub-bands plots for GNR and CNT devices
    type = 1;
end

%Points in GNR/CNT width
NW = input('Enter number of Unit Cells in the width : ');
if isempty(NW)
    NW = 12;
end

%Points in GNR/CNT length
NL = 1;
%Points in GNR/CNT unit cell
NU = 4;
...
if (device == 1)
    fprintf(1,'Device: Graphene Nano-Ribbon\n'); 
else
    fprintf(1,'Device: Carbon Nanotube\n'); 
end
...
%Armchair Structure for CNT
    betaw(4,1) = t;
    betal(2,1) = t; betal(3,4) = t;
else
%Zigzag Structure for CNT
    betaw(2,1) = t; betaw(3,4) = t;
    betal(4,1) = t;
end
...
if (device == 2)
alpha = alpha+kron(diag(ones(1,1),NW-1),betaw')+kron(diag(ones(1,1),1- NW),betaw);
end
